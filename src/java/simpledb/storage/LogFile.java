...
//    int pageSize;
    ...
    */
    //rollback是undo log做的事，即提供上一个版本的快照（相比MVCC真是微不足道），
    // 在回滚时将上一个版本的数据写回磁盘
    //1.根据tidToFirstLogRecord获取该事务第一条记录的位置；
    //2.移动到日志开始的地方,
    //3.根据日志格式进行读取日志记录，
    // 读到update格式的记录时根据事务id判断是否为要修改的日志，如果是，写before image
    ...
    */
    //崩溃恢复是redo log要做的事，在因故障数据丢失时，
    // 有部分数据是还未写入数据库的，这个时候可以利用到undo log。从日志文件中，
    // 我们可以获取到checkpoint所在位置，
    // 然后对checkpoint后面的日志记录进行读取并进行恢复数据
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                //存放事务id对应的beforePage和after page
