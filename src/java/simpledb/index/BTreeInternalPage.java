...
//B+树的内部节点
	...
	//其实内部节点可以看成保存着一个个BTreeEntry，
// 内部节点对key的查找、插入、删除、迭代，都是以entry为单位的。
...
public class BTreeInternalPage extends BTreePage {
	...
	private final int[] children;//存储page的序号，用于每个key指向左右children的btreePageId。也因此如果keys是m，children则是m+1。
	private final int numSlots;//内部节点中最多能存储指针的数量
	
	private int childCategory; // either leaf or internal，孩子节点的类型：叶子或内部节点
	 ...
	 */
//	/对于BTreeInternalPage来说keys，childern在lab中初始的大小都相同，
//	只是开始赋值的索引不相同：
