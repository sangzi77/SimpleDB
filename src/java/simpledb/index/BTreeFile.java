...
public class BTreeFile implements DbFile {
	...
	private final TupleDesc td;//tupledesc
	...
	//————————————————
	//findLeafPage() 函数，递归获取叶子节点
	private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
                                       Field f)
					throws DbException, TransactionAbortedException {
		// some code goes here
		// 四种节点情况：ROOT_PTR(根节点) 、INTERNAL 、LEAF、HEADER;
		int type = pid.pgcateg();//用于标识BTreePage的类型
		if(type == BTreePageId.LEAF){//如果类型是叶节点
			//则直接返回
		...
		while (it.hasNext()){
			...
			// 找到第一个大于（或等于）filed的entry，然后递归其左孩子；
	 ...
	 */
	//分裂叶子节点
	//当页节点中 元组数量等于n 时，将其拆分成两个页节点。返回插入tuple所在的page。
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		BTreeLeafPage newRightPage = (BTreeLeafPage)
				getEmptyPage(tid,dirtypages,BTreePageId.LEAF);

		//2、将当前page中一半的tuple插入到newRightPage中。
		// 插入时应该先从page中删除tuple，然后再插入到newRightPage。
		// （newRightPage插入tuple后会给其赋值新的recordId，
		// page删除tuple时根据其recordId进行查找然后删除，
		// 而page无法定位到被赋值了新recordId的tuple，则无法将其删除）。
		...
		//3、如果当前page有右兄弟oldRightPage，
		// 将oldRightPage左兄弟的指针指向newRightPage，
		// 将newRightPage的右兄弟指针指向oldRightPage。
		// 并将oldRightPage添加到dirtypages中。

		//检查原页面是否有右页面，如果有需要更新指针
		...
		//4、将page的右兄弟指针指向newRightPage，
		// newRightPage的左兄弟指针指向page。
		//将page、newRightPage添加到dirtypages中。
		...
		//5、获取指向该page的内部节点，
		// 在其中添加一个指向page和newRightPage的新entry。
		// 将父entry所在的page添加到dirtypages中。

		//getParentWithEmptySlots；获取具有读写权限的父页面，如果父节点中key的数量到达了n-1，
		// 则会调用splitInternalPage()方法向上递归，总之最终会返回一个可以插入新key的内部节点。

		//找出父节点并创建entry进行插入，最后标记脏页（比较容易理解的结论）
	 ...
	 */
	//这里是内部页节点，上面的是叶页节点
	//实现与splitLeafPage类似，区别是“分裂”之后要将中间的key“挤到” 父节点中去。
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		//3、分配完entry后，选出page中最大的entry，
		// 将其从page中删除，并将该entry的左孩子指针指向page，右孩子指针指向newRightPage，
	 ...
	 * @throws TransactionAbortedException
	 */
	//调用getPage（）获取page时，将检查页面是否已经存储在本地缓存dirtypage中，
	// 如果不再，则调用BufferPool.getPage去获取。
	// getPage（）如果使用读写权限获取页面，也会将页面添加到dirtypages缓存中，
	// 因为它们可能很快就会被弄脏。
	// 这种方法的一个优点是，如果在一个元组插入或删除过程中多次访问相同的页面，
	// 它可以防止更新丢失。
	//
	Page getPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm)
		...
		if(dirtypages.containsKey(pid)) {
			...
			//page不在本地缓存dirtypage中，去bufferpool中取
			Page p = null;
			p = Database.getBufferPool().getPage(tid, pid, perm);
			if(perm == Permissions.READ_WRITE) {//使用读写权限获取页面，也会将页面添加到dirtypages缓存中
	 ...
	 * @throws DbException
	 */
	//尝试从小于半满的叶页面中删除元组会导致该页面从其兄弟之一窃取元素或与其兄弟之一合并。
	// exercise3中就需要实现叶节点和内部节点从兄弟节点“窃取”元素的功能。

	//页面元素的重新分配
	//nternal page的entry，
	// leaf page的tuple，两种页面的分配是有区别的，
	// 重新分配tuple的话parent是需要包含tuple的value的，
	// 而重新分配entry是不需要的，因为真正的数据是在leaf page中，
	// 这个只是作为一个索引，当然你加上也是没问题的，
	// 但是这样做是会更方便的，至少图示就是这样做的。
	//————————————————
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		//到时候忘了的话，去这里看看图示就明白了
		//https://blog.csdn.net/weixin_43414605/article/details/124207926?spm=1001.2014.3001.5501
		...
		//3、参数entry是父节点中指向page和其兄弟节点的entry，
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		//2、因为内部节点与其父节点中的key值没有重复
		// ，迁移key的时候也需要将父节点中的key移动到page中。
		BTreeEntry entry = it.next();
