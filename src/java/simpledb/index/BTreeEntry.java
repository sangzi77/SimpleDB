 ...
 *
 */
// BTreeInternalPage所更新的单位，虽然BTreeInternalPage页面中存储的是keys与children，
// 但是实际更新（查找、插入、删除等）的单位则是BTreeEntry对象。

// BTreeInternalPage所更新的单位，
// 虽然BTreeInternalPage页面中存储的是keys与children，
// 但是实际更新（查找、插入、删除等）的单位则是BTreeEntry对象。
public class BTreeEntry implements Serializable {
	 ...
	 * */
	private Field key;//entry的key

	/**
	 * The left child page id
	 * */
	private BTreePageId leftChild; // 左孩子的page id

	/**
	 * The right child page id
	 * */
	private BTreePageId rightChild; // 右孩子的page id

	/**
	 * The record id of this entry
	 * */
	private RecordId rid;//记录entry位于哪个page
	// null if not stored on any page
