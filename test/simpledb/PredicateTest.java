...
public class PredicateTest extends SimpleDbTestBase{
  ...
  public void filter() {
    ...
    for (int i : vals) {
      Predicate p = new Predicate(0, Predicate.Op.EQUALS, TestUtil.getField(i));//创建一个Predicate：下标0的位置，等于，创建一个-1字段
      assertFalse(p.filter(Utility.getHeapTuple(i - 1)));
      assertTrue(p.filter(Utility.getHeapTuple(i)));//与值为-1的int型字段比较
      assertFalse(p.filter(Utility.getHeapTuple(i + 1)));
    }
